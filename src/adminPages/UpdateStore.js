...
function UpdateStore() {
     ...
     const updateStore = async () => {
          ...
          let result = await Fetch(host + `/v1/admin/stores/${param.id}/update`, "POST", formData);
               ...
               <AdminHeader placeholder="Search by name, email..." />
               {
                    wait ?
                         ...
                         <div className="w-4/5 float-left">
                              <h1 className="flex font-bold text-3xl ml-5 mt-7"><FormattedMessage id='updateStore' /></h1>
                              <section className="bg-purple-200 w-11/12 mt-10 rounded-l-lg py-3 float-right relative">
                                   ...
                                   <div className="flex justify-around max-sm:block max-sm:ml-3 max-sm:text-sm">
                                        <div className="w-2/5 flex flex-col max-sm:w-4/5">
                                             ...
                                             <input ref={nameEnRef} defaultValue={location.state.name_en} type="text" placeholder="Enter english name" className="w-full h-10 indent-2 rounded-md outline-none" />
                                        </div>
                                        <div className="w-2/5 flex flex-col max-sm:w-4/5">
                                             <div className="font-bold w-fit mb-2 max-sm:max-sm:my-2"><FormattedMessage id='arName' /></div>
                                             <input ref={nameArRef} defaultValue={location.state.name_ar} type="text" placeholder="Enter arabic name" className="w-full h-10 indent-2 rounded-md outline-none" />
                                        </div>
                                   </div>
                                   ...
                                   <button onClick={updateStore} className="my-8 rounded-md bg-white text-purple-500 px-7 py-2 cursor-pointer font-semibold hover:border hover:border-white hover:text-white hover:bg-purple-200 duration-200">
                                        {
